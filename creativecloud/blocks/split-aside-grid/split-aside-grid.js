...
const CHEVRON_SVG = '<svg viewBox="0 0 12 12"><path d="M4 1l5 5-5 5" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
function setupBlock(el) {
  ...
  function applyRotation() {
    ...
    dotEls.forEach((dot, idx) => {
      dot.classList.toggle('is-active', slotOf(idx) === 0);
    });
  }
  ...
  function handleNavigation(e) {
    if (e.target.classList.contains('prev')) commitRight(0.25, true);
    else if (e.target.classList.contains('next')) commitLeft(0.25);
  }
...
function decorate(block) {
  ...
  const dots = createTag('ul', { class: 'split-aside-grid-dots' });
  medias.forEach(() => {
    dots.appendChild(createTag('li', { class: 'split-aside-grid-dot' }));
  });
  ...
  const controlsWrapper = createTag(
    ...
    [
      createTag('button', { class: 'prev' }, ARROW_SVG),
      dots,
      createTag('button', { class: 'next' }, ARROW_SVG),
    ],
  );
  ...
  block.append(itemsWrap, controlsWrapper, stack);
